...
function LogMealModal({ onClose }) {
  ...
  return (
    <>
      ...
      {/* Sheet */}
      <div style={{ position:'fixed', left:0, right:0, bottom:0, zIndex:201, background:'#fff', borderRadius:'24px 24px 0 0', maxHeight:'88vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ padding:'16px 20px 12px', borderBottom:'1px solid rgba(26,22,38,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <p style={{ fontSize:'16px', fontWeight:800, color:'#1A1626', letterSpacing:'-.02em', margin:0 }}>
            {selected ? `${selected.name}` : 'Choose a recipe'}
          </p>
          <button onClick={onClose} style={{ background:'#F0EEF8', border:'none', borderRadius:'50%', width:'32px', height:'32px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <X size={16} strokeWidth={2.5} color='#7B728E' />
          </button>
        </div>
        ...
        {!selected ? (
          <>
            {/* Search */}
            <div style={{ padding:'12px 16px', borderBottom:'1px solid rgba(26,22,38,0.05)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', background:'#F8F7FD', borderRadius:'14px', padding:'10px 14px' }}>
                <Search size={15} strokeWidth={2} color='#B4ADCA' />
                <input
                  autoFocus
                  value={query}
                  onChange={e => setQuery(e.target.value)}
                  placeholder="Search 220 recipes…"
                  style={{ flex:1, background:'none', border:'none', outline:'none', fontSize:'14px', color:'#1A1626', fontFamily:'Plus Jakarta Sans, sans-serif' }}
                />
              </div>
            </div>
            ...
            <div style={{ overflowY:'auto', flex:1, WebkitOverflowScrolling:'touch' }}>
              {filtered.slice(0, 60).map(r => (
                <button key={r.id} onClick={() => setSelected(r)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 20px', background:'none', border:'none', cursor:'pointer', borderBottom:'1px solid rgba(26,22,38,0.04)', fontFamily:'Plus Jakarta Sans, sans-serif', textAlign:'left' }}>
                  <div>
                    <p style={{ fontSize:'13px', fontWeight:700, color:'#1A1626', margin:'0 0 2px', letterSpacing:'-.01em' }}>{r.name}</p>
                    <p style={{ fontSize:'10px', color:'#B4ADCA', margin:0, fontWeight:500 }}>{r.protein}g P · {r.carbs}g C · {r.fat}g F</p>
                  </div>
          ...
          <div style={{ padding:'20px', display:'flex', flexDirection:'column', gap:'16px' }}>
            {/* Macro summary */}
            <div style={{ background:'#F8F7FD', borderRadius:'16px', padding:'16px', display:'flex', justifyContent:'space-around' }}>
              <div style={{ textAlign:'center' }}>
                <p style={{ fontSize:'20px', fontWeight:800, color:'#4F3FD4', letterSpacing:'-.04em', margin:0 }}>{selected.cal}</p>
                <p style={{ fontSize:'10px', color:'#B4ADCA', fontWeight:500, margin:'2px 0 0' }}>cal</p>
              </div>
              {[{l:'Protein',v:selected.protein,c:'#4F3FD4'},{l:'Carbs',v:selected.carbs,c:'#0DC8A0'},{l:'Fat',v:selected.fat,c:'#F5A623'}].map(m => (
                <div key={m.l} style={{ textAlign:'center' }}>
                  ...
                  <p style={{ fontSize:'10px', color:'#B4ADCA', fontWeight:500, margin:'2px 0 0' }}>{m.l}</p>
                </div>
              ))}
            </div>
            ...
            {/* Slot picker */}
            <div>
              <p style={{ fontSize:'12px', fontWeight:700, color:'#7B728E', marginBottom:'10px', letterSpacing:'.04em', textTransform:'uppercase' }}>Meal slot</p>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'8px' }}>
                {SLOTS.map(s => (
                  <button key={s} onClick={() => setSlot(s)} style={{
                    ...
                    background: slot === s ? '#4F3FD4' : '#F0EEF8',
                    color:      slot === s ? '#fff'    : '#7B728E',
            ...
            <div style={{ display:'flex', gap:'10px' }}>
              <button onClick={() => setSelected(null)} style={{ flex:1, padding:'14px', borderRadius:'14px', border:'1.5px solid rgba(79,63,212,0.15)', background:'#fff', cursor:'pointer', fontSize:'13px', fontWeight:700, color:'#7B728E', fontFamily:'Plus Jakarta Sans, sans-serif' }}>
                Back
              </button>
      ...
      <div style={{ paddingBottom:'88px' }}>
        ...
        <div style={{ padding:'0 16px', marginTop:'-12px', position:'relative', zIndex:10 }}>
          ...
          <div style={{ background:'#fff', borderRadius:'24px', padding:'20px', boxShadow:'0 4px 24px rgba(79,63,212,0.12)', marginBottom:'12px' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
              <div>
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:'4px' }}>
                  ...
                  <span style={{ fontSize:'14px', color:'#B4ADCA', fontWeight:500 }}>/ {goals.calories}</span>
                </div>
              </div>
            ...
            <p style={{ fontSize:'11px', color:'#B4ADCA', fontWeight:500, marginTop:'6px', textAlign:'right' }}>
          ...
          <div style={{ background:'#fff', borderRadius:'24px', padding:'20px', boxShadow:'0 4px 24px rgba(79,63,212,0.12)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'14px' }}>
              <p style={{ fontSize:'14px', fontWeight:800, color:'#1A1626', letterSpacing:'-.02em', margin:0 }}>Logged today</p>
            ...
            {meals.length === 0 ? (
              <div style={{ textAlign:'center', padding:'24px 0' }}>
                <p style={{ fontSize:'13px', color:'#B4ADCA', fontWeight:500 }}>Nothing logged yet today</p>
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                {meals.map((m) => (
                  <div key={m.id} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px', background:'#F8F7FD', borderRadius:'14px' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'2px' }}>
                        <p style={{ fontSize:'13px', fontWeight:700, color:'#1A1626', letterSpacing:'-.01em', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{m.recipe.name}</p>
                      ...
                      <p style={{ fontSize:'10px', color:'#B4ADCA', fontWeight:500, margin:0 }}>{m.slot} · {m.time} · {m.recipe.protein}g P · {m.recipe.carbs}g C · {m.recipe.fat}g F</p>
                    </div>
